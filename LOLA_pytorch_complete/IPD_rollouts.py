...
def get_rollouts(policy1, policy2, r1arr=[-1, -3, 0, -2], r2arr=[-1, 0, -3, -2], rollout_length=100, num_rollout=20, gamma=0.8, verbose=False):
    ...
    for _ in range(num_rollout):
        ...
        for t, transition in enumerate(trajectory):
            ...
            R1 += __grad(policy1, u1, s) * (__future_discounted_R(trajectory, t, 0) - b(0)) * gamma**t
            R2 += __grad(policy2, u2, s) * (__future_discounted_R(trajectory, t, 1) - b(1)) * gamma**t

            # based on Eq. (4.5) LOLA paper (12 Dec 17)
